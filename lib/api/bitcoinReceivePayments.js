...
class BitcoinGateway extends EventEmitter {
    ...
    async checkAddress(transaction) {
        ...
        console.log(`found ${tx.data.confirmed_received_value} total coins sent for ${transaction.address.toString()}`)

        // unconfirmed value can = confirmations

        if (tx.data.confirmed_received_value >= transaction.total) {
